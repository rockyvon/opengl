...
map<string, Model*> ModelLoader::loadedModels;
...
Model* ModelLoader::loadModel(string path)
{
	init();
	modelPath = path;
	map<string, Model*>::iterator it = loadedModels.find(path);
	loaded = it != loadedModels.end();
	if (loaded) {
		return it->second;
	}
	return NULL;
...
Model* AssimpModelLoader::loadModel(string path)
{
...
string AssimpModelLoader::getBasePath(string path)
{
	size_t pos = path.find_last_of("\\/");
	return (std::string::npos == pos) ? "" : path.substr(0, pos + 1);
}

void AssimpModelLoader::processAllTextures(const aiScene * scene, bool gamma)
{
	...
	for (i = 0; i < scene->mNumMaterials; i++) {
		...
		while (scene->mMaterials[i]->GetTexture(aiTextureType_DIFFUSE, texIndex, &aipath) == AI_SUCCESS)
		{
			textureMap[aipath.data] = NULL; //fill map with textures, pointers still NULL yet
			texIndex++;
		}
		texIndex = 0;
		while (scene->mMaterials[i]->GetTexture(aiTextureType_SPECULAR, texIndex, &aipath) == AI_SUCCESS)
		{
			textureMap[aipath.data] = NULL; //fill map with textures, pointers still NULL yet
			texIndex++;
		}
	...
	for (int i = 0; i<numTextures; i++)
	{
		...
		stringReplace(path, "\\", "/");
		std::string fileLocation = basepath + path;  // get filename
...
Material AssimpModelLoader::processMaterial(aiMaterial * aimaterial)
{
	int ret1, ret2;
	aiColor4D diffuse;
	aiColor4D specular;
...
void AssimpModelLoader::processNode(const aiNode* ainode, const aiScene* scene, Model* model)
{
	...
	for (i = 0; i < ainode->mNumMeshes; i++) {
		...
		model->meshes->push_back(Mesh(vertices, faces, material));
	}

	for (i = 0; i < ainode->mNumChildren; i++) {
		processNode(ainode->mChildren[i], scene, model);
	}
}

void AssimpModelLoader::arrayToVec4(vec4 & v, float * array)
{
	v.x = array[0];
	v.y = array[1];
	v.z = array[2];
...
void AssimpModelLoader::aiColor4ToVec4(vec4 & v, aiColor4D & aicolor)
{
	v.x = aicolor.r;
	v.y = aicolor.g;
	v.z = aicolor.b;
