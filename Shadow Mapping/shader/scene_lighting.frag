...
float calculate_shadow(sampler2D texture_depth);
...
void main()
{
	...
	//FragColor = vec4(vec3(pow(gl_FragCoord.z, 16)) + 1 / 2, 1.0);
	//FragColor = texture(material.difTex, vfdata.texCoords);
	
}

float calculate_shadow(sampler2D texture_depth, vec3 direction)
{
	vec3 division = vfdata.lightCoordsPostion.xyz / vfdata.lightCoordsPostion.w;
	division = division * 0.5 + 0.5;
	float min_depth = texture(texture_depth, division.xy).r;
	float bias = max(0.003 * (1.0 - dot(vfdata.normal, direction)), 0.003);
	return min_depth > (division.z - bias)?0.0:1.0;
	//return 1.0;
}
...
vec3 calculate_light_directional(LightDirectional light, vec3 normal, vec3 view_direcion, vec3 amb_color, vec3 dif_color, vec3 spe_color)
{
	...
	float shadow = calculate_shadow(depthMap, -light.direction);
